...
涉及到贷款平台，首先要有评分（一般的信贷平台都用的对齐300-850这类的评分卡标准，我们也套用了），然后要进行分析，才能完成贷前/中/后期的管理。经过讨论，组员们给出了两个大致要实现的模型：
...
核心实现步骤：
- 计算若干buckets，每个子项给出 points（正/负分）。
- 求和得到rawPoints，然后把rawPoints线性映射到 [300,850]。
...
后续是打算从规则判断转向ML，其实就是做什么逻辑回归之类的比较高大上的东西（老师喜欢），但是讲多了会很像是AI泔水，不在此过度阐释了。