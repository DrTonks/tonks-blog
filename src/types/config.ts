...
export type SiteConfig = {
	...
	navbarTitle?: {
		...
		url?: string; // 顶栏标题链接，默认指向站点根目录
		external?: boolean; // 是否作为外部链接打开
	};
	...
	banner: {
		...
		waves?: {
			...
			defaultEnabled?: boolean; // 用户尚未选择时是否默认播放
			allowUserToggle?: boolean; // 是否允许访客在显示设置中切换
...
export type AnnouncementConfig = {
	...
	version?: string; // 可选显式版本；未提供时自动使用公告内容指纹
	title?: string; // 公告栏标题
