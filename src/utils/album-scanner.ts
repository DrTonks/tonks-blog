...
async function processAlbumFolder(
	...
	return {
		...
		cover,
		// 使用统一的日期格式化函数，优先使用 info.date，其次使用当前日期
		date: info.date
			? formatDateToYYYYMMDD(new Date(info.date))
...
function processExternalPhotos(
	...
	externalPhotos.forEach((photo, index) => {
		...
		photos.push({
			...
			tags: photo.tags || [],
			// external photo 的日期使用提供的日期（若有），否则使用当前日期
			date: photo.date
				? formatDateToYYYYMMDD(new Date(photo.date))
				: formatDateToYYYYMMDD(new Date()),
