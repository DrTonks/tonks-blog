import {mountPoll} from '@blog/src/utils/poll-ui.js';
...
export async function renderPreview(context){
 const {text,file}=context;
   const rendered=await renderMarkdown(text,{sourceMap:context.live===true});
   ...
   for(const card of doc.querySelectorAll('post-reference')){
    ...
    if(data?.image&&match){const image=doc.createElement('img');image.src=await context.resolveAsset(data.image,match);image.alt='';link.append(image);}else {const placeholder=doc.createElement('span');placeholder.className='post-card-placeholder';placeholder.textContent='↗';link.append(placeholder);}link.append(copy);for(const key of ['data-preview-from','data-preview-to'])if(card.hasAttribute(key))link.setAttribute(key,card.getAttribute(key));card.replaceWith(link);
   }
   for(const card of doc.querySelectorAll('github')){const repo=card.getAttribute('repo')||'';const link=doc.createElement('a');link.className='preview-github';link.href='https://github.com/'+repo;link.textContent='GitHub · '+repo+' ↗';for(const key of ['data-preview-from','data-preview-to'])if(card.hasAttribute(key))link.setAttribute(key,card.getAttribute(key));card.replaceWith(link);}
   ...
   const blocks=context.live?[...doc.body.children].filter(el=>el.hasAttribute('data-preview-from')).map(el=>({from:Number(el.getAttribute('data-preview-from')),to:Number(el.getAttribute('data-preview-to')),html:el.outerHTML})):undefined;
   ...
   const draw=()=>{const dark=context.theme.startsWith('dark'),yellow=context.theme.endsWith('yellow');
    return `<!doctype html><html class="${dark?'dark':''}" data-yellow="${yellow}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data: https: http:; media-src data: https: http:; style-src 'unsafe-inline'; font-src data:; script-src 'nonce-tonks-preview';"><style>${katexCss}\n${css.replace(/<\/style/gi,'<\\/style')}</style></head><body><article id="post-container"><h1 class="preview-title">${title}</h1><div class="custom-md">${content}</div></article><script nonce="tonks-preview">document.querySelectorAll(".article-poll").forEach(card=>{const mount=${mountPoll.toString()};mount(card,async option=>{const ids=[...card.querySelectorAll("[data-option]")].map(e=>e.dataset.option);return option?{voted:true,choice:option,total:1,counts:Object.fromEntries(ids.map(id=>[id,id===option?1:0])),answer:card.dataset.pollAnswer,explanation:card.dataset.pollExplanation}:{voted:false};},new AbortController().signal);});document.querySelectorAll('.spoiler-trigger').forEach(b=>b.addEventListener('click',()=>{const p=b.parentElement,c=p.querySelector('.spoiler-content');p.classList.toggle('is-revealed');const open=p.classList.contains('is-revealed');b.setAttribute('aria-expanded',open);c?.setAttribute('aria-hidden',!open);if(c)c.inert=!open;}));document.querySelectorAll('a').forEach(a=>a.addEventListener('click',e=>{const h=a.getAttribute('href');if(h?.startsWith('#'))return;e.preventDefault();}));</script></body></html>`;
   };return {html:draw(),missing,blocks};
}
