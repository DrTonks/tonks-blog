...
上述操作不启动 Astro、不构建整站、不部署。JSON 和 CSS 更新无需这些命令。构建出的 adapter 会被插件按内容变化重新加载，不需要重新安装或重启 Obsidian。已打开的预览是快照，必须返回编辑再打开。
...
适配器导出 `apiVersion = 1` 和异步 `renderPreview(context)`，返回 `{ html, missing }`。宿主提供编辑器快照、当前文件、主题、配置、本地资源读取和文章查找；适配器负责站点专属渲染，HTML 展示在不授予同源权限的 iframe 中。
