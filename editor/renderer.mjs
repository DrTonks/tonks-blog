...
const processor=unified().use(parse).use(gfm).use(math).use(directive).use(remarkPolls,{preview:true}).use(parseDirectiveNode).use(toHast,{allowDangerousHtml:true}).use(raw).use(slug).use(components).use(articleMedia).use(tableCards).use(katex).use(stringify);
