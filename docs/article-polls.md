...
增加 `answer="a" explanation="答案解析"` 即为答题模式。2–8 个选项，单选，投票后展示结果；两个选项额外展示左右对比条。ID 全站唯一且永久稳定，选项 ID 不随排序变化。已同步题目可以润色标题、修改解析、调整选项顺序，沿用原 ID 保留票数和讨论身份；选项 ID、选项文字、正确答案及投票／答题类型仍冻结，修改时须使用新 ID。标题修改若改变题意，也应主动使用新 ID。文案修订生成新版本，前端发布后启用；回滚恢复旧文案，票数不回退。此规则需要先部署兼容后端，再运行 `pnpm ship`；同步失败不会发布前端，字段冲突会指出投票 ID 和字段名，不输出答案或解析。没有验证码登录，限制的是共享 Cookie 身份，无法阻止清除身份后再次投票。
