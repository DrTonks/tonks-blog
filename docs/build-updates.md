...
- SFTP 在本地构建及文件清单检查成功后，递归删除远程目标目录并重新创建，再上传 dist 内容；旧资源及已删除页面不会保留。
- 上传失败不发布后续阶段；清空后上传期间站点可能暂时不可用，失败后需重新部署，不会恢复旧站点。
- CLEAN_REMOTE/cleanRemote 已弃用并忽略，SFTP 固定执行清空后上传。
